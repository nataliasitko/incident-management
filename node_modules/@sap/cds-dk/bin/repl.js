...
async function repl (_, options={}) {
  ...
  repl.defineCommand ('run', {
    help: 'Runs a cds server in the given CAP project folder, which can be a module name like @capire/bookshop.',
    async action (project) {
      // await cds.exec ('--project', project, '--port', '0')
      await cds.test (project)
      console.log ('Following are made avaliable as global variables:')
      console.log ('- cds.services:', Object.keys(cds.services)); Object.assign (repl.context, cds.services)
      console.log ('- cds.entities:', Object.keys(cds.entities)); Object.assign (repl.context, cds.entities)
    }
  })
